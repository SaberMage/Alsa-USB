...
BGM_TEST="$(pgrep -f audacious > /dev/null && echo 1 || echo 0)"
CARD_EVENT=0 #Default

#End script if auto-switching is off
[ -f $AU_VARS ] && . $AU_VARS && [ -f $AUTO_DISABLE ] && exit
...
while [ 1 ]; do
	...
	if [ $CARD_EVENT == 1 ]; then
		...
		if [ -f $GAME_DISABLE ] && [ $BGM_TEST == 1 ]; then
			printf "\n>> Passed BGM test\n"
			$ALSASWITCH $SOUND_CARD &> /dev/null &
			$(HOME=/home/pi && $BGM_PLAYER -i &> /dev/null &)
		else
			#If playing game, change sound card when game ends
			while [ -f $GAME_DISABLE ]; do sleep 0.2; done
